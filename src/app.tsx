...
function Parent() {
  const [iframeOnSameOrigin, setIframeOnSameOrigin] = useState<boolean>(true);
  ...
  return (
    <>
      <div className="relative flex flex-row gap-20">
        <iframe
          src={iframeOnSameOrigin ? window.location.href : 'https://atlassian.design'}
          width={600}
          height={600}
        />
        <div
          className={`bg-slate-200 p-2 rounded border-2 border-black flex flex-col gap-2 w-80 h-80 ${
            isOnTop ? 'absolute shadow-lg left-60 top-20' : ''
          }`}
        >
          <span>
          ...
          <label className="flex flex-row gap-2">
            <input
              type="checkbox"
              checked={iframeOnSameOrigin}
              onChange={() => setIframeOnSameOrigin(!iframeOnSameOrigin)}
            />
